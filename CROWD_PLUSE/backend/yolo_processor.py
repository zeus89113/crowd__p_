...
def process_video_file(video_path, output_folder, live_update_callback=None):
    """
    Processes a video, sends live updates via a callback, extracts final analytics,
    ...
    while cap.isOpened():
        ...
        # --- NEW: Send live update via callback ---
        if live_update_callback:
            live_update_callback(person_count)
    ...
    command = [
        'ffmpeg', '-i', temp_output_path, '-vcodec', 'libx264',
        '-pix_fmt', 'yuv420p', '-movflags', '+faststart', '-y', final_output_path
    ]
